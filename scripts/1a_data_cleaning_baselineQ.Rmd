...
updated July 14th to just pull from Longitudinal Project.
...
```{r, echo = F}
phase1_df <- read.csv("../raw_data/survey_data/COPECOVID19Perinatal_DATA_INACTIVE_Baseline_use.csv") 
...
```{r, echo = F}
span_df <- read.csv("../raw_data/survey_data/COPECOVID19YExperien_DATA_span_use.csv") 
...
new_df <- read.csv("../raw_data/survey_data/COPECOVID19Perinatal_DATA_EXPANDED_Baseline_use.csv") 
new_df2 <- new_df %>%
  filter(consent_agree == "Yes") %>%
  select(-contains("timestamp")) %>%
   mutate(child_birth_date = as.character(child_birth_date), 
...
duplicates <- read.csv("../raw_data/survey_data/started_new_survey_already_enrolled.csv")
nrow(duplicates)
...
# make subsets of scales 
bsi_somat_scale <- baseline_clean %>%
  select(bsi_weak_body, bsi_nausea, 
                               bsi_numbness, bsi_faint_dizzy, bsi_breathless, bsi_heart_pain) %>%
    mutate_if(is.numeric, funs (. -1)) 

bsi_dep_scale <- baseline_clean %>%
  select(bsi_feeling_blue , bsi_no_interest , 
         bsi_feel_lonely , bsi_hopeless , bsi_worthless) %>%
    mutate_if(is.numeric, funs (. -1)) 

bsi_anx_scale <- baseline_clean %>%
  select(bsi_feel_tense , bsi_nervous_shaky , bsi_fearful ,
         bsi_scared , bsi_restless , bsi_terror_spells) %>%
    mutate_if(is.numeric, funs (. -1)) 
...
bsi_data <- baseline_clean %>%
  ...
  mutate_if(is.numeric, funs (. -1)) %>%
  # calculate scores - taking averages because some items may have been skipped!
  # shold be 18 questions but the suicidal thoughts not included.
  mutate(bsi_global = rowSums(., na.rm = T)/17,
         bsi_somatization = rowSums(bsi_somat_scale)/6,
         # note: do not have suicidal thoughts Q 
         bsi_depression = rowSums(bsi_dep_scale)/5,
         bsi_anxiety = rowSums(bsi_anx_scale)/6) %>%
  ...
  select(record_id, bsi_global, bsi_somatization, bsi_depression, bsi_anxiety)

summary(bsi_data$bsi_somatization)
...
save(baseline_clean2, file = "../cope_data_dashboard/clean_data/baseline_deidentified_df.Rdata")
