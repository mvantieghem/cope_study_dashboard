...
```{r, echo = F}
phase1_df <- read.csv("raw_data/COPECOVID19Perinatal_DATA_use.csv") 
...
new_df <- read.csv("raw_data/COPECOVID19Perinatal_DATA_use.csv") 
new_df2 <- new_df %>%
  filter(consent_agree == "Yes") %>%
  select(-contains("timestamp")) %>%
   mutate(child_birth_date = as.character(child_birth_date), 
...
### check N
```{r, eval = F}
print(paste0("Total Records:", nrow(new_df)))
print("Consented:")
...
## save baseline data
```{r}
save(baseline_clean2, file = "clean_data/baseline_clean_df.Rdata")
